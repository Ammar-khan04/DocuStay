    ...
    if code:
        ...
        if inv and (getattr(inv, "invitation_kind", None) or "").strip().lower() != "guest":
            ...
            if not awaiting_signup and (
                inv.status not in ("pending", "ongoing") or tok == "BURNED"
            ):
                inv = None
    sig = None
...
def register_guest(request: Request, data: GuestRegister, db: Session = Depends(get_db)):
    ...
    if code and target_role == UserRole.guest:
        ...
        tok = (inv.token_state or "").upper()
        if not awaiting_signup and (
            inv.status not in ("pending", "ongoing", "expired") or tok == "BURNED"
        ):
            create_log(
                db,
                CATEGORY_FAILED_ATTEMPT,
...
def accept_invite(
    ...
    if is_tenant_invite:
        ...
        if inv.status not in ("pending", "ongoing"):
            ...
            db.commit()
            raise HTTPException(status_code=400, detail="Invalid or expired invitation code")
        if inv.status in ("pending", "ongoing") and (getattr(inv, "token_state", None) or "") == "BURNED":
            create_log(
                db,
                CATEGORY_FAILED_ATTEMPT,
