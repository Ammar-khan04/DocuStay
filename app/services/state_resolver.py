...
AssignmentStatus = Literal["none", "future", "active", "ended"]
...
def resolve_assignment_status(ta: TenantAssignment | None, *, today: date | None = None) -> AssignmentStatus:
    if ta is None:
        return "none"
    today = today or date.today()
    start = getattr(ta, "start_date", None)
    end = getattr(ta, "end_date", None)
...
def resolve_tenant_state(
    ...
    *,
    ...
    asg_status = resolve_assignment_status(tenant_assignment)
